...
extern "C"
    ...
    espeak_TextToPhonemesWithTerminator(const void **textptr, int textmode, int phonememode, int *terminator);
/* Translates text into phonemes.  Call espeak_SetVoiceByName() first, to select
   ...
   multi-letter phonemes names bits 8-23:  separator character, between phoneme
   names

   terminator: A pointer to an integer that will be set to the terminator
